...
- [x] Record and freeze the post-audit policy revision.
  - Frozen: `autonomy_policy_v2`, assembled 2026-08-17. It inherits every
    `autonomy_policy_v1` rule except that the extensive-change displacement
    threshold is evaluated over the **sum of all moved-block displacement in
    one reconciliation run**, not each block independently. More than 1,440
    aggregate minutes requires renewed approval. The unchanged limits of two
    changed blocks, preferred-focus placement, and daily focus minutes still
    apply. A mutation targeting an event without valid CommitmentOS ownership
    is now represented by the explicit `forbidden` disposition; it can never
    be converted to automatic by an approval override. Runtime audit records
    pin `policy_thresholds_v2`; new environments pin
...
| 2026-08-17 | **Post-audit autonomy policy revision frozen**: `autonomy_policy_v2` replaces the per-block 24-hour displacement test with a per-run aggregate 1,440-minute test and makes invalid mutation ownership an explicit, non-overridable `forbidden` disposition. The v1 artifact and all historical evidence remain unchanged. This revision is locally regression-tested but is not represented by the 2026-08-15 deployed evidence campaign. | Autonomy-policy block above; `docs/post_audit_hardening_progress.md`; `backend/tests/unit/test_phase4b_repair.py` | Build session; owner-run deployment/live gate pending |
| TBD | TBD | TBD | TBD |
